...
def Conv2DTrans_Layer(m,nfilter,kernel,strides):
    m.add(Conv2DTranspose(nfilter,kernel, strides=strides, padding='same', data_format="channels_first",
                            kernel_initializer=weightInit))
    return m

def ConvT_bn(m, nfilter,kernel,strides):
    m = Conv2DTrans_Layer(m, nfilter,kernel,strides)
...
def build_generator(latent_size):
    ...
    cnn.add(Dense(1024 * idim/16 * idim/16,input_dim=latent_size))
    cnn.add(LeakyReLU())
    cnn.add(Reshape((1024, idim/16, idim/16)))

    cnn = ConvT_bn(cnn,512,(5,5),(2,2))
    cnn = ConvT_bn(cnn,256,(5,5),(2,2))
    cnn = ConvT_bn(cnn,128,(5,5),(2,2))
    cnn = Conv2DTrans_Layer(cnn,3,(5,5),(2,2))
...
def Conv2D_Layer(m,nfilter,kernel,strides, inputShape = None):
    if inputShape == None:
        m.add(Conv2D(nfilter,kernel, strides=strides, padding='same', data_format="channels_first",
                            kernel_initializer=weightInit))
    else:
        m.add(Conv2D(nfilter,kernel, strides=strides, padding='same', data_format="channels_first",
                            kernel_initializer=weightInit,input_shape=inputShape))
    return m

def add_bn_lr_drop(m,do_prob):
    m.add(BatchNormalization(axis=1))
    m.add(LeakyReLU())
    m.add(Dropout(do_prob))
    return m

def Conv2D_bn_lr_drop(m, nfilter,kernel,strides,do_prob):
    m = Conv2D_Layer(m, nfilter,kernel,strides)
    m = add_bn_lr_drop(m,do_prob)
    return m
...
def build_discriminator():
    ...
    cnn = Conv2D_Layer(cnn,64,(3,3),(2,2),(imaChan, imageDim, imageDim))
    cnn = add_bn_lr_drop(cnn,doProb)

    cnn = Conv2D_bn_lr_drop(cnn,64,(3,3),(2,2),doProb)
    cnn = Conv2D_bn_lr_drop(cnn,128,(3,3),(2,2),doProb)
    cnn = Conv2D_bn_lr_drop(cnn,256,(3,3),(2,2),doProb)
...
if __name__ == '__main__':
    ...
    noiseExamples = np.tile( np.random.normal(0, 1, (10, latent_size)) ,(numClass,1))
